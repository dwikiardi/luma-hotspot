...
async def create_fingerprint(payload: FingerprintPayload):
    ...
    # Canvas fingerprint (high entropy) - up to +15
    ...
    # WebGL fingerprint (high entropy) - up to +15
    ...
    # Timezone consistency - up to +5
    if payload.timezone:
        signals["timezone"] = {"weight": 5, "value": payload.timezone}
        score += 5

    # Platform/OS info - up to +5
    ...
    try:
        ...
        cur.execute("""
            ...
            (fingerprint_hash, visitor_id, ip_address, mac, nas_id, user_agent, 
             ...
             trust_score, risk_factors, created_at)
